...
public class Topic extends Model {
    ...
    public Long getVoicesCount() {
        return User.count("select count(distinct u) from User u, Topic t, Post p where p.postedBy = u and p.topic = t and t = ?0", this);
    }

    public Post getFirstPost() {
       return Post.find("topic = ?0 order by postedAt asc", this).first();
   }
   
    public Post getLastPost() {
        return Post.find("topic = ?0 order by postedAt desc", this).first();
    }
